...
export class LogoScreen extends Container {
  ...
  constructor() {
    super();
    // z-order bottom → top: ecgGfx, auraGfx, logo (show), liveDot (show), liveText (show)
  ...
  private drawLiveIndicator(cx: number, cy: number): void {
    ...
    const dotX = cx + LOGO_SIZE * 0.5 + 114;
    this.liveDot.x  = dotX;
    this.liveDot.y  = cy;
    this.liveText.x = dotX + 14 + LOGO_SIZE * 0.1;
    this.liveText.y = cy;
    ...
    this.liveDot.clear();
    this.liveDot.circle(0, 0, 5).fill({ color: 0xff3333, alpha });
    this.liveDot.circle(0, 0, 10).fill({ color: 0xff3333, alpha: alpha * 0.22 });
