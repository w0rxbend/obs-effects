...
export class GalaxyBgScreen extends Container {
  ...
  constructor() {
    super();
    [
      this.bgG, this.haloLayer, this.dustLayer,
      this.armLayer, this.nebulaLayer,
      this.coreGlowG, this.coreLayer, this.spikeG,
    ].forEach(l => this.addChild(l));

    // All rotating layers pivot around canvas center
    for (const layer of [
      this.haloLayer, this.dustLayer, this.nebulaLayer,
      this.armLayer, this.coreGlowG, this.coreLayer, this.spikeG,
    ]) {
  ...
  public async show(): Promise<void> {
    const palette = [
      PINK, MAUVE, LAVENDER, BLUE, SAPPHIRE, TEAL,
      YELLOW, PEACH, FLAMINGO, GREEN, SURFACE2,
    ];
    this.texMap = new Map();
  ...
  public update(time: Ticker): void {
    ...
    for (const nb of this.nebMeta) {
      nb.c.alpha = 0.04 + 0.05 * (0.5 + 0.5 * Math.sin(TAU * this.elapsed / nb.period + nb.phase));
    }
  }
  ...
  private buildBg(): void {
    ...
    for (let i = 0; i < N_BG; i++) {
      g.circle(rnd(0, W), rnd(0, H), 0.6)
       .fill({ color: i % 2 === 0 ? SURFACE1 : SURFACE2, alpha: rnd(0.10, 0.40) });
    }
    for (let i = 0; i < N_BRIGHT; i++) {
      g.circle(rnd(0, W), rnd(0, H), 0.9)
       .fill({ color: SURFACE2, alpha: rnd(0.45, 0.75) });
    }
  }
  ...
  private buildArms(): void {
    ...
    for (let a = 0; a < 4; a++) {
      ...
      for (let p = 0; p < counts[a]; p++) {
        ...
        const angle     = offsets[a] + theta + (gaussian() * sw * 0.25) / Math.max(r_base, 80);
        ...
        let al: number;
        let sc: number;
        if      (t < 0.30) { color = Math.random() < 0.5 ? PINK     : MAUVE;    al = rnd(0.45, 0.82); sc = rnd(0.38, 0.68); }
        else if (t < 0.65) { color = Math.random() < 0.5 ? LAVENDER : BLUE;     al = rnd(0.35, 0.72); sc = rnd(0.28, 0.52); }
        else               { color = Math.random() < 0.5 ? SAPPHIRE : TEAL;     al = rnd(0.22, 0.52); sc = rnd(0.22, 0.42); }
  ...
  private buildNebulae(): void {
    const pairs: [number, number][] = [
      [PINK, FLAMINGO], [TEAL, GREEN],    [BLUE, SAPPHIRE],
      [MAUVE, LAVENDER],[PINK, MAUVE],    [TEAL, SAPPHIRE],  [FLAMINGO, PEACH],
    ];
    for (let n = 0; n < 7; n++) {
      const t      = rnd(0.1, 0.9);
      const theta  = t * 1.5 * TAU;
      ...
      for (let i = 0; i < Math.floor(rnd(80, 120)); i++) {
        g.circle(nx + gaussian() * 55, ny + gaussian() * 55, rnd(15, 35))
         .fill({ color: Math.random() < 0.5 ? c1 : c2, alpha: 1.0 });
      }
      nb.addChild(g);
      nb.alpha = rnd(0.04, 0.08);
      this.nebulaLayer.addChild(nb);
      this.nebMeta.push({ c: nb, phase: rnd(0, TAU), period: rnd(5000, 12000) });
    }
  }
  ...
  private buildCore(): void {
    for (let i = 0; i < N_CORE; i++) {
      ...
      const color = t < 0.5
        ? (Math.random() < 0.5 ? YELLOW : PEACH)
        : (Math.random() < 0.5 ? MAUVE  : PINK);
      const s = new Sprite(this.texMap.get(color)!);
      s.anchor.set(0.5);
      s.x     = CX + Math.cos(angle) * r;
      s.y     = CY + Math.sin(angle) * r;
  ...
  private buildSpikes(): void {
    ...
    for (let i = 0; i < N; i++) {
      ...
      do { x = rnd(60, W - 60); y = rnd(60, H - 60); }
      ...
      for (let k = 0; k < 4; k++) {
        ...
        for (const dir of [-1, 1] as const) {
          g.moveTo(x, y)
           .lineTo(x + Math.cos(ang) * dir * len, y + Math.sin(ang) * dir * len)
